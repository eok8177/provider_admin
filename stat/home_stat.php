...
<!-------------------------------------------------------------->
    ...
    <form action="?mod=stat" method="post">
...
<?
  ...
  echo "<tr><td><b>Всего :</b></td><td> $summ грн</td></tr>";?>
    ...
    <table border=1 cellspacing=0>
    <tr><td>Время входа</td><td>Длительность</td><td>Отослано</td><td>Принято</td><td>С IP адресса</td><td>счёт</td></tr>

<?
  for ($i=0; $i<mysql_num_rows($que3); $i++) {
    $s=mysql_fetch_array($que3);
	echo "<tr>";
    echo "<td><SMALL>$s[start_time]</SMALL></td><td>$s[connect_time]</td><td>$s[in_b]</td><td>$s[out_b]</td><td><SMALL>$s[from_IP]</SMALL></td><td><SMALL>$s[acct]</SMALL></td>";
	echo "</tr>\n";
					    }
?>
...
<? echo "<br>In : $in MБайт Получено от пользователя<br>Out: $out MБайт Отослано пользователю<br>";?>
